...
a.
```cpp
chan atencion(int); // canal para pasar el id del cliente y que lo atienda el empleado

Process Cliente [id: 0..N-1] {
    send atencion(id); // n clientes envían su id al empleado
}

Process Empleado {
    int idAux;
    while (true) { // el empleado atiende indefinidamente
        receive atencion(idAux); // espera a que el canal tenga un id de cliente
        atender(idAux); // atiende al cliente
    }
}
```
...
b.
```cpp
// no cambia nada, solo se agrega un segundo empleado que atiende de la misma forma
chan atencion(int);
...
c.
```cpp
// se necesita un proceso adicional que administre los pedidos de los empleados
chan atencion(int);
chan pedido(int);
chan siguiente[2](int); // canal para que el admin le diga al empleado qué cliente atender
...
Process Admin {
    ...
    while (true) { // el admin atiende indefinidamente
        receive pedido(idE); // espera a que un empleado le pida un cliente
        if (empty (atencion)) { // si no hay clientes esperando
            idC = -1; // le dice al empleado que no hay clientes
        } else { 
            receive atencion(idC); //recibe el id del siguiente cliente
            send siguiente[idE](idC); // le pasa el id del siguiente cliente al empleado que le pidió
        }
    }
}
...
Process Empleado [id: 0..1] {
    int idC;
    while (true) {
        send pedido(id); // le pide un cliente al admin
        receive siguiente[id](idC); // espera a que el admin le diga qué cliente atender
        if (idC == -1) { // si no hay clientes esperando realiza tareas administrativas
           delay(900)
        } else { // si hay un cliente lo atiende
            atender(idC);
        }
    }
}
